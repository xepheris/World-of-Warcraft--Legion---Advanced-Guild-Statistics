	...
	if($data != '') {		
		...
		if($escaped_session_guild_name == $escaped_source_guild_name) {
			...
			if($data['level'] == '110') {
				...
				if($class == '11') {
					$eq_ap = (($artifact_power/$hreshold*$worth)/4*3)/2.5;
				}
				elseif($class == '12') {
				...
				$sum = $brh_mythic+$cen_mythic+$dht_mythic+$eoa_mythic+$hov_mythic+$lkz_mythic+$mos_mythic+$nl_mythic+$arc_mythic+$vh_mythic+$ukz_mythic+$votw_mythic;
				$m0 = $sum-$mythic_plus2;
				$m2_to_m5 = $mythic_plus2-$mythic_plus5;
				$m5_to_m10 = $mythic_plus5-$mythic_plus10;
				$m10_to_15 = $mythic_plus10-$mythic_plus15;
				...
				$eq = $world_quests*1
					...
					+$m10_to_15*$eq_weights['m10_15']
					+$mythic_plus15*$eq_weights['m15p']
				...
				// GENERAL INFORMATION
				$update_guild_table = mysqli_query($stream, "UPDATE `" .$table_name. "` SET `logout` = '" .$logout. "', `updated` = '" .time('now'). "', `talents` = '" .$talent_calc_var. "', `eq` = '" .$eq. "' WHERE `name` = '" .$character. "'");
